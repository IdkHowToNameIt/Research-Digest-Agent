/* ---------------------------------------------------------------------------
   ...
   selezionabile e il file pesa poco. jsPDF arriva da npm ed è impacchettata nel
   bundle: nessuna CDN a runtime, come prima con la copia vendorizzata.

   NB versione: si è passati da jsPDF 2.5.2 a 4.x durante la migrazione a React,
   perché la 2.5.2 ha una vulnerabilità critica (ReDoS) e veniva servita così com'era
   dal sito. L'API usata qui (text, splitTextToSize, addImage, output, save) è
   invariata tra le due, ed è coperta dai test in pdf.test.js.
--------------------------------------------------------------------------- */
import { jsPDF } from 'jspdf'
import LOGO_SRC from './assets/kva-logo.webp'
import { fmtData, fmtLettura, nomeTema, pad2 } from './dati.js'
  ...
  // Oltre questo tempo si rinuncia al logo e si genera comunque il PDF. Senza il
  // timeout, un'immagine che non arriva MAI (richiesta bloccata, o un ambiente in
  // cui Image non emette eventi — jsdom fa esattamente questo) lascerebbe il
  // bottone a girare per sempre, senza errore e senza documento.
  ...
  function caricaLogo(){
    ...
    return new Promise(function(resolve){
      ...
      setTimeout(function(){ chiudi(false); }, LOGO_TIMEOUT_MS);
      const img = new Image();
      img.onload = function(){
        let esito;
        try{
          ...
          esito = {dataUrl: c.toDataURL('image/png'), w: c.width, h: c.height};
        }catch(e){ esito = false; }   // canvas "tainted" o webp non decodificabile
        chiudi(esito);
      };
      img.onerror = function(){ chiudi(false); };
      img.src = LOGO_SRC;
    });
  }
  ...
  function costruisci(doc, t, g, logo){
    ...
    const tema = nomeTema(t.id) || t.nome || t.id;
    const data = fmtData(g.data);
    const lett = fmtLettura(g.minuti);
...
/* --------- API del modulo -------------------------------------------------
   Prima queste funzioni leggevano lo stato globale di app.js e manipolavano il
   DOM. Ora ricevono i dati come argomenti e restituiscono un risultato: la parte
   di interfaccia (bottoni, popover, messaggi) vive nei componenti React. */

/** Costruisce il PDF del digest di un giorno. Ritorna {doc, filename}. */
export async function creaDocGruppo(tema, gruppo, articoli) {
  if (!tema || !gruppo || !articoli || !articoli.length) {
    throw new Error('Nessun digest aperto.');
  }
  const logo = await caricaLogo();
  const doc = new jsPDF({ unit: 'mm', format: 'a4' });
  costruisci(doc, tema, { ...gruppo, articoli }, logo);
  return { doc, filename: nomeFile(tema, gruppo) };
}

/** Genera il PDF e lo scarica in locale. */
export async function scaricaPdf(tema, gruppo, articoli) {
...
/** Genera il PDF e lo invia al Worker, che inoltra a Resend. Nessun SMTP lato
 *  client e nessuna chiave qui: l'API key vive solo nell'env del Worker.
 *
 *  Ritorna {ok:true} oppure {ok:false, errore:'...'} — non solleva per gli errori
 *  attesi (rete, indirizzo rifiutato), così il chiamante mostra un messaggio
 *  invece di un'eccezione. */
export async function inviaPdfEmail(url, email, tema, gruppo, articoli, fetchImpl = fetch) {
  if (!url) return { ok: false, errore: 'Invio email non configurato.' };
  if (!EMAIL_VALIDA.test(email)) return { ok: false, errore: 'Inserisci un indirizzo email valido.' };

  let doc, filename;
  try {
    ({ doc, filename } = await creaDocGruppo(tema, gruppo, articoli));
  } catch (e) {
    return { ok: false, errore: (e && e.message) || 'PDF non generato.' };
  }

  try {
    // base64 puro (senza il prefisso "data:...;base64,") per l'allegato Resend
    ...
    let dati = {};
    try { dati = await resp.json(); } catch (e) { /* risposta non JSON: si tratta come KO */ }
    if (resp.ok && dati.ok) return { ok: true };
    return { ok: false, errore: dati.errore || 'Invio non riuscito. Riprova.' };
  } catch (e) {
    return { ok: false, errore: 'Rete non disponibile. Riprova.' };
  }
}

// esportate per i test: sanifica è il cuore del fix sui caratteri (DECISIONI §16)
export { sanifica, nomeFile };
